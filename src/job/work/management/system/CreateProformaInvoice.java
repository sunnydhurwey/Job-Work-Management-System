 ...
 * @author sunny
 */
public class CreateProformaInvoice extends javax.swing.JFrame {
    ...
    private static CreateProformaInvoice obj = null;

    public static CreateProformaInvoice getObj() {
        if (obj == null) {
            obj = new CreateProformaInvoice();
        }
        return obj;
    }
...
//            cmbAddress.setEnabled( false );
    ...
    public void getData() {
        try {
            ...
            dbrate = 0;
            dbqty = 0;
            String sql = "SELECT * FROM proformainvoicemaster WHERE invoiceno='" + txtInvoiceNo.getText() + "'";
            pst = conn.prepareStatement( sql );
            rs = pst.executeQuery();
            while (rs.next()) {
...
//Function or method to get saved invoice data calculation value from database
    public void getInvoiceDataCALC() {
        try {
            String sql = "SELECT * FROM proformainvoice WHERE invoiceno='" + txtInvoiceNo.getText() + "'";
            pst = conn.prepareStatement( sql );
            rs2 = pst.executeQuery();
            if (rs2.next()) {
    ...
    public void getQuotationDataCALC() {
        try {
            String sql = "SELECT * FROM proformainvoice WHERE challanno='" + txtChallanNo.getText() + "'";
            pst = conn.prepareStatement( sql );
            rs2 = pst.executeQuery();
            if (rs2.next()) {
...
//                cmbClientName.setSelectedItem( rs2.getString( "clientname" ) );
    ...
    public void saveInvoice() {
        try {
            String sql = "INSERT INTO proformainvoice (challanno,date,company,address,basicprice,"
    ...
    public void updateInvoice() {
        try {
            String sql = "UPDATE proformainvoice SET date=?,company=?,address=?,basicprice=?,discount=?,gstper=?,gstamt=?,totalamt=?,"
    ...
    public void deleteInvoice() {
        try {
            String sql = "DELETE FROM proformainvoice WHERE invoiceno='" + txtInvoiceNo.getText() + "'";
            pst = conn.prepareStatement( sql );
            pst.execute();
        } catch (SQLException e) {
...
//        }
    ...
    public void importJobOrder() {
        try {
            String sql = "INSERT INTO proformainvoicemaster (joborderno,date,productname,processname,materialname,tstrt,w,od,tl,mdp,discountper,qty,rate,"
...
//Function or method to get Invoice Table Data To Field
    ...
    public void getInvoiceTableDataToField() {
        try {
            ...
            String sql = "SELECT * FROM proformainvoicemaster WHERE inv_uid='" + tblInvClick + "'";
            pst2 = conn.prepareStatement( sql );
            rs2 = pst2.executeQuery();
            if (rs2.next()) {
                cmbProductName.setSelectedItem( rs2.getString( "productname" ) );
    ...
    public void removeImport() {
        try {
            String sql = "DELETE FROM proformainvoicemaster WHERE inv_uid='" + tblInvClick + "'";
            pst = conn.prepareStatement( sql );
            pst.execute();
    ...
    public void getInvoiceDataToTable() {
        try {
            String sql = "SELECT inv_uid AS 'INV_UID',invoiceno AS 'INVOICE NO',joborderno AS 'JOB ORDER NO',productname AS 'PRODUCT NAME',processname AS 'PROCESS NAME',"
                    ...
                    + "discountper AS 'DISC. %',discountamt AS 'DISC. AMT',amount AS 'AMOUNT' FROM proformainvoicemaster WHERE invoiceno='" + txtInvoiceNo.getText() + "'";
            pst = conn.prepareStatement( sql );
            rs = pst.executeQuery();
    ...
    public void calcInvoiceTableData() {
        try {
            double d = 0, a = 0;
            String sql = "SELECT discountamt,amount  FROM proformainvoicemaster WHERE invoiceno='" + txtInvoiceNo.getText() + "'";
            pst = conn.prepareStatement( sql );
            rs = pst.executeQuery();
            while (rs.next()) {
    ...
    private void initComponents() {
        ...
        setTitle("PROFORMA INVOICE BUILDER - JOB WORK MANAGEMENT SYSTEM");
        ...
        jLabel23.setText("PROFORMA INVOICE NO.");
        ...
        setSize(new java.awt.Dimension(1178, 644));
...
obj=null;
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger( CreateProformaInvoice.class
        ...
        java.awt.EventQueue.invokeLater( new Runnable() {
            public void run() {
                new CreateProformaInvoice().setVisible( true );
            }
        } );
    }
